...
function completionsEndpoint(baseUrl) {
  return candidateEndpoints(baseUrl)[0];
}

function candidateEndpoints(baseUrl) {
...
function buildTranslateMessages(texts, targetLang, sourceLang = "auto") {
  const target = langLabel(targetLang);
  const from =
    sourceLang && sourceLang !== "auto" ? `from ${langLabel(sourceLang)} ` : "";
  ...
  return [
    ...
    {
      role: "user",
      content: `Translate every item ${from}into ${target}.
...
function buildSummarizeMessages(page, targetLang) {
  const target = langLabel(targetLang);
  return [
    {
      role: "system",
...
root.KeliApi = { completionsEndpoint, candidateEndpoints, langLabel, extractJsonArray, chatCompletions, buildTranslateMessages, buildSummarizeMessages };
